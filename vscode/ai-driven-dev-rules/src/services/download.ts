...
import { IGitHubApiService } from "../api/github"; // Import GitHub service interface
import { GithubRepository } from "../api/types"; // Import Repository type
...
export class DownloadService implements IDownloadService {
	...
	public async downloadFiles(
		...
		if (files.length === 0 || !repository) {
			this.logger.warn("downloadFiles called with no files or no repository info.");
			return [];
		}
		...
		return vscode.window.withProgress(
			...
			async (progress, token) => {
				...
				const startNextDownload = async (): Promise<void> => {
					...
					if (!file) { // Only check if queue is empty
						return;
					}
					...
					try {
						...
						if (contentData.download_url) {
							...
							this.logger.debug(
								`Using download_url for ${file.targetPath}`,
							);
							...
							this.logger.debug(
								`Using base64 content for ${file.targetPath}`,
							);
