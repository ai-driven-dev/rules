...
export class GithubExplorerItem extends vscode.TreeItem {
  ...
  constructor(content: GithubContent, parent?: GithubExplorerItem) {
  ...
  private updateParentSelection(): void {
    ...
    const childStates = this.parent.children.map(
      (child) => child.selectionState
    );
    const selectedCount = childStates.filter(
      (state) => state === SelectionState.Selected
    ).length;
    const indeterminateCount = childStates.filter(
      (state) => state === SelectionState.Indeterminate
    ).length;
