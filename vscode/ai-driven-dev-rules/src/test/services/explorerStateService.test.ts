...
import { ExplorerStateService, IExplorerStateService } from "../../services/explorerStateService";
...
describe("ExplorerStateService", () => {
	...
	const createMockExplorerItem = (path: string, type: 'file' | 'dir' = 'file'): ExplorerTreeItem => {
		return {
			content: { path, type, name: path.split('/').pop() || path, sha: `sha-${path}`, size: 0, url: "", html_url: "", git_url: "", download_url: null },
			label: path.split('/').pop() || path,
            collapsibleState: type === 'dir' ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None,
		} as unknown as ExplorerTreeItem;
	};

	const repo1: GithubRepository = { owner: "owner1", name: "repo1", branch: "main" };
	const repo2: GithubRepository = { owner: "owner2", name: "repo2", branch: "dev" };
	...
	describe("Repository State", () => {
		it("should set and get the current repository", () => {
			...
			sinon.assert.calledWith(mockLogger.debug, sinon.match(/Setting repository state/));
		});
    ...
    describe("Root Loading State", () => {
        it("should set and get root loading state", () => {
            ...
            sinon.assert.calledWith(mockLogger.debug, sinon.match(/Setting root loading state to: true/));
            stateService.setRootLoading(false);
            expect(stateService.isRootLoading()).to.be.false;
            sinon.assert.calledWith(mockLogger.debug, sinon.match(/Setting root loading state to: false/));
        });
    ...
    describe("Root Items State", () => {
        it("should set and get root items", () => {
            ...
            expect(stateService.getRootItems()).to.deep.equal(rootItems);
             sinon.assert.calledWith(mockLogger.debug, sinon.match(/Setting root items state \(count: 2\)/));
        });
        ...
        it("should allow setting root items to null", () => {
            ...
            expect(stateService.getRootItems()).to.be.null;
            sinon.assert.calledWith(mockLogger.debug, sinon.match(/Setting root items state \(count: null\)/));
        });
    });
    ...
    describe("Directory Loading Promises", () => {
        ...
        it("should set and get loading promises", () => {
            ...
            sinon.assert.calledWith(mockLogger.debug, `Setting loading promise for path: ${dirPath}`);
        });
        ...
        it("should delete loading promises", () => {
            ...
            expect(stateService.getLoadingPromise(dirPath)).to.be.undefined;
            sinon.assert.calledWith(mockLogger.debug, `Deleting loading promise for path: ${dirPath}`);
        });
        ...
        it("should clear all loading promises", () => {
            ...
            sinon.assert.calledWith(mockLogger.debug, "Clearing directory loading promises.");
        });
