...
import { RateLimitManager, IRateLimitManager } from "../../services/rateLimitManager";
...
describe("RateLimitManager", () => {
	...
	describe("updateFromHeaders", () => {
		it("should update rate limit info from valid headers", () => {
            ...
            sinon.assert.calledWith(mockLogger.debug, sinon.match(/Rate limit updated/));
		});
        ...
        it("should not update or log if headers provide the same info", () => {
            ...
            const info = rateLimitManager.getRateLimitInfo();
			expect(info).to.deep.equal({ // Info should still be the same
				limit: 5000,
				remaining: 4999,
				reset: resetTimeFuture,
			});
		...
		it("should handle headers with array values (taking the first)", () => {
			const headersArray: http.IncomingHttpHeaders = {
				...
				"x-ratelimit-reset": [resetTimeFuture.toString(), (resetTimeFuture + 100).toString()],
			};
        ...
        it("should log a warning if headers are missing after initial update", () => {
            ...
            expect(rateLimitManager.getRateLimitInfo()).to.deep.equal({ // State remains unchanged
                limit: 5000,
				remaining: 4999,
				reset: resetTimeFuture,
            });
