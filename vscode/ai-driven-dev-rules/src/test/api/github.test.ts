...
import type { GithubContent, GithubRepository } from "../../api/types"; // Import GithubContent
...
describe("GitHubApiService", () => {
  ...
  beforeEach(() => {
    ...
    getConfigurationStub
      .withArgs("aidd")
      .returns(mockConfiguration as unknown as vscode.WorkspaceConfiguration);
  ...
  describe("fetchRepositoryContentRecursive", () => {
    ...
    it("should fetch branch details and recursive tree successfully without auth token", async () => {
      // Arrange
      mockHttpClient.get.withArgs(branchApiUrl, sinon.match.any).resolves({
        statusCode: 200, // Correct property name
        headers: {},
        body: JSON.stringify(mockBranchResponse), // Body should be string
      });
      mockHttpClient.get.withArgs(treeApiUrl, sinon.match.any).resolves({
        statusCode: 200, // Correct property name
        headers: {},
      ...
      // Act
      const result =
        await githubApiService.fetchRepositoryContentRecursive(testRepo);

      // Assert
      expect(result.success).to.be.true;
      if (!result.success) {
        throw new Error("Assertion failed: result should be success");
      } // Type guard
      ...
      sinon.assert.calledWith(
        ...
        branchApiUrl,
        sinon.match(
          (headers: Record<string, string>) => !headers.Authorization,
        ),
      );
      sinon.assert.calledWith(
        mockHttpClient.get,
      ...
      const file1 = result.data.find(
        (item: GithubContent) => item.path === "file1.txt",
      );
      ...
      const dir1 = result.data.find(
        (item: GithubContent) => item.path === "dir1",
      );
      expect(dir1?.type).to.equal("dir");
      expect(dir1?.name).to.equal("dir1");
      expect(dir1?.download_url).to.be.null;

      const file2 = result.data.find(
        (item: GithubContent) => item.path === "dir1/file2.ts",
      );
      ...
      sinon.assert.calledWith(
        mockLogger.info,
        sinon.match(/Fetching recursive tree/),
      );
      sinon.assert.calledWith(
        mockLogger.debug,
        sinon.match(/Found root tree SHA/),
      );
      sinon.assert.calledWith(
        mockLogger.info,
        sinon.match(/Recursive tree fetch complete/),
      );
    ...
    it.skip("should fetch branch details and recursive tree successfully with auth token", async () => {
      ...
      // Act
      const result =
        await authGithubApiService.fetchRepositoryContentRecursive(testRepo);
      ...
      sinon.assert.calledWith(
        mockLogger.debug,
        sinon.match(/GitHub API Request \(Auth\)/),
      );

      // Verify the token was used in the headers
      const authHeader = `token ${token}`;
      const headerMatcher = sinon.match(
    ...
    it("should handle truncated tree response and log warning", async () => {
      ...
      mockHttpClient.get.withArgs(treeApiUrl, sinon.match.any).resolves({
        statusCode: 200,
        headers: {},
        body: JSON.stringify(truncatedTreeResponse),
      });
      ...
      sinon.assert.calledWith(
        mockLogger.warn,
        sinon.match(/response for recursive tree was truncated/),
      );
    ...
    it("should return error if fetching branch details fails", async () => {
      ...
      if (result.success) {
        throw new Error("Assertion failed: result should be error");
      } // Type guard
      ...
      sinon.assert.calledWith(
        mockLogger.error,
        sinon.match(/Failed to fetch branch details/),
      );
      sinon.assert.notCalled(
        mockHttpClient.get.withArgs(treeApiUrl, sinon.match.any),
      ); // Tree API should not be called
    });
    ...
    it("should return error if tree SHA is missing in branch response", async () => {
      ...
      mockHttpClient.get.withArgs(branchApiUrl, sinon.match.any).resolves({
        statusCode: 200,
        headers: {},
        body: JSON.stringify(malformedBranchResponse),
      });
      ...
      sinon.assert.calledWith(
        mockLogger.error,
        sinon.match(/Could not find tree SHA/),
      );
      sinon.assert.notCalled(
        mockHttpClient.get.withArgs(treeApiUrl, sinon.match.any),
      );
    ...
    it("should return error if fetching recursive tree fails", async () => {
      ...
      sinon.assert.calledWith(
        mockLogger.error,
        sinon.match(/Failed to fetch recursive tree/),
      );
    });
    ...
    it("should return error if http client throws during branch fetch", async () => {
      ...
      sinon.assert.calledWith(
        mockLogger.error,
        sinon.match(/HTTP Client error.*branches/),
        sinon.match.any,
      );
      sinon.assert.notCalled(
        mockHttpClient.get.withArgs(treeApiUrl, sinon.match.any),
    ...
    it("should return error if http client throws during tree fetch", async () => {
      ...
      sinon.assert.calledWith(
        mockLogger.error,
        sinon.match(/HTTP Client error.*git\/trees/),
        sinon.match.any,
      );
    });
  });
  ...
  describe("fetchFileContent", () => {
    ...
    it("should return error if http client throws", async () => {
      ...
      mockHttpClient.get
        .withArgs(downloadUrl, sinon.match.any)
        .rejects(clientError);
