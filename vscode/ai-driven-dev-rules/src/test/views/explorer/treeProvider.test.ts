...
describe("ExplorerTreeProvider", () => {
	...
	let mockEventEmitter: sinon.SinonStubbedInstance<vscode.EventEmitter<ExplorerTreeItem | undefined | null | void>>;
	let expect: Chai.ExpectStatic;

	const testRepo: GithubRepository = { owner: "test-owner", name: "test-repo", branch: "main" };
	const mockExtensionPath = "/mock/extension/path";

	// Helper to create basic mock ExplorerTreeItem
	const createMockExplorerItem = (content: GithubContent, parent?: ExplorerTreeItem): ExplorerTreeItem => {
		return {
			content,
			parent,
			label: content.name,
			collapsibleState: content.type === 'dir' ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None,
	...
	beforeEach(() => {
        ...
        mockGithubService = { // Define the stub object directly
        ...
        mockStateService = { // Define the stub object directly
		...
		mockEventEmitter = sinon.createStubInstance<vscode.EventEmitter<ExplorerTreeItem | undefined | null | void>>(vscode.EventEmitter);
	...
	describe("getChildren - Root Loading & Transformation", () => {
		...
		it("should trigger background load and return loading placeholder if root is not loaded", async () => {
			...
			// Add missing git_url
			const loadingPlaceholder = createMockExplorerItem({ name: "Loading...", path: "loading", type: "file", sha: "", size: 0, url: "", html_url: "", git_url: "", download_url: null });
        ...
        it("should process flat Git Tree data into hierarchical items and cache them", async () => {
            ...
            const flatContent: GithubContent[] = [
                { name: "file1.txt", path: "file1.txt", type: "file", sha: "f1", size: 10, url: "", html_url: "", git_url: "", download_url: "url1" },
                { name: "dir1", path: "dir1", type: "dir", sha: "d1", size: 0, url: "", html_url: "", git_url: "", download_url: null },
                { name: "file2.ts", path: "dir1/file2.ts", type: "file", sha: "f2", size: 20, url: "", html_url: "", git_url: "", download_url: "url2" },
                { name: "file3.md", path: "file3.md", type: "file", sha: "f3", size: 30, url: "", html_url: "", git_url: "", download_url: "url3" },
                { name: "dir2", path: "dir2", type: "dir", sha: "d2", size: 0, url: "", html_url: "", git_url: "", download_url: null },
                { name: "subfile.js", path: "dir1/subfile.js", type: "file", sha: "f4", size: 40, url: "", html_url: "", git_url: "", download_url: "url4" },
            ];
            mockGithubService.fetchRepositoryContentRecursive.resolves({ success: true, data: flatContent });
            ...
            mockTreeItemFactory.createItem.callsFake((content: GithubContent, parent?: ExplorerTreeItem) => {
            ...
            mockStateService.setRootItems.callsFake((items: ExplorerTreeItem[] | null) => {
            ...
            flatContent.forEach(content => {
                ...
                if (content.path.includes('/')) {
                    const parentPath = content.path.substring(0, content.path.lastIndexOf('/'));
            ...
            const expectedRootItems = Array.from(createdItemsMap.values()).filter(item => !item.content.path.includes('/'));
            ...
            // Assert
            sinon.assert.calledOnce(mockGithubService.fetchRepositoryContentRecursive.withArgs(testRepo, "", sinon.match.number));
            expect(mockTreeItemFactory.createItem.callCount).to.equal(flatContent.length);
            ...
            // Verify setRootItems call
             ...
             if (setRootItemsArgs) { // Add null check for type safety
                 const rootPaths = setRootItemsArgs.map((item: ExplorerTreeItem) => item.content.path);
                 expect(rootPaths).to.have.members(["file1.txt", "dir1", "file3.md", "dir2"]);
    ...
    describe("handleCheckboxChange", () => {
        it("should call selectionService.toggleRecursiveSelection for a directory", () => {
            // Add missing git_url
            const dirItem = createMockExplorerItem({ name: "dir1", path: "dir1", type: "dir", sha: "d1", size: 0, url: "", html_url: "", git_url: "", download_url: null });
            treeProvider.handleCheckboxChange(dirItem, true); // 'checked' value is currently ignored by the method
            sinon.assert.calledOnceWithExactly(mockSelectionService.toggleRecursiveSelection, "dir1");
            sinon.assert.notCalled(mockSelectionService.toggleSelection);
        });

        it("should call selectionService.toggleSelection for a file", () => {
             // Add missing git_url
             const fileItem = createMockExplorerItem({ name: "file1.txt", path: "file1.txt", type: "file", sha: "f1", size: 10, url: "", html_url: "", git_url: "", download_url: "url1" });
            treeProvider.handleCheckboxChange(fileItem, true); // 'checked' value is currently ignored by the method
            sinon.assert.calledOnceWithExactly(mockSelectionService.toggleSelection, "file1.txt");
     ...
     describe("getTreeItem", () => {
        it("should call updateSelectionState with correct state", () => {
            ...
            sinon.assert.calledOnceWithExactly(fileItem.updateSelectionState as sinon.SinonStub, true);
        });

         it("should fallback to setting checkboxState if updateSelectionState is missing", () => {
            // Add missing git_url
            const fileItem = createMockExplorerItem({ name: "file1.txt", path: "file1.txt", type: "file", sha: "f1", size: 10, url: "", html_url: "", git_url: "", download_url: "url1" });
            delete (fileItem as any).updateSelectionState; // Remove the method
